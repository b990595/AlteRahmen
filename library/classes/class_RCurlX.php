...
Class RCurlX {
    ...
    private $_maxConcurrent = 8; //max. number of simultaneous connections allowed
    private $_options = []; //shared cURL options
    private $_headers = []; //shared cURL request headers
    private $_timeout = 30; //all requests must be completed by this time
    private $requests = []; //request_queue
    private $results = [];
    private $_preUrl = "";
    
    private $jsonEncodePostFields = false;

    public function __construct($jsonEncodePostFields = false) {
        $this->jsonEncodePostFields = $jsonEncodePostFields;
    }
    ...
    public function setMaxThreads($maxThreads) {
        if ($maxThreads > 0) {
            $this->_maxConcurrent = $maxThreads;
        }
    }
    ...
    public function GET($url, $data = NULL, array $options = NULL, array $headers = NULL) {
        return $this->addRequest("GET", $url, $data, $options, $headers);
    }

    public function POST($url, $data = NULL, array $options = NULL, array $headers = NULL) {
        return $this->addRequest("POST", $url, $data, $options, $headers);
    }

    public function PUT($url, $data = NULL, array $options = NULL, array $headers = NULL) {
        return $this->addRequest("PUT", $url, $data, $options, $headers);
    }

    public function DELETE($url, $data = NULL, array $options = NULL, array $headers = NULL) {
        return $this->addRequest("DELETE", $url, $data, $options, $headers);
    }

    private function addRequest($method, $url, $data = NULL, array $options = NULL, //individual cURL options
            array $headers = NULL //individual cURL request headers
    ) { //Add to request queue
        $tmpIndex = count($this->requests);
        ...
        return $tmpIndex; //return request number/index
    }
    ...
    public function execute() {
        ...
        // Benyttes til dannelse af RCurlResult
        $origRequests = $this->requests;
        ...
        for ($i = 0; $i < min(count($this->requests), $this->_maxConcurrent); $i++) {
        ...
        $retur = array();
        if (is_array($returnResults)) {
            for ($x = 0; $x < count($returnResults); $x++) {
                $retur[] = new RCurlResult($returnResults[$x]['response'], $returnResults[$x]['info'], $origRequests[$x]['method'], $origRequests[$x]['url']);
            }
        }
        return $retur;
    }
    ...
    private function process_request($completed, $multi_handle, array &$requests_map) {
        ...
        $this->results[$user_data] = array("response" => $response, "info" => $request_info);
